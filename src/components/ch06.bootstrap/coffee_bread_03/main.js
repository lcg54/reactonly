...
function App() {
  ...
  const selectedProduct = products.filter(product => product.id === clickedProductRowId)[0]; // 객체 1개를 저장하고 있는 배열로 반환되므로, [0]으로 첫번째 항목을 반환


  const ModeChanged = (mode) => setMode(mode);
  ...
  return (
    <>
      ...
      <Card>
        ...
        <Card.Footer>
          <Bottom onClickToBottom={ModeChanged} />
        </Card.Footer>
      </Card>
      <br /><br />
    </>
